...
(** [solve_mon_with IH] extends the library's [solve_mon] tactic with a
    user-supplied induction hypothesis [IH] for resolving recursive
...
(** Monotonicity of [ftailA].  The proof structure mirrors [headA_mon]:
    ...
    the Undefined case via [solve_mon], then reduce to a statement
    about bare [SeqA A] values which is proved by induction on the
    upper bound [q2'] using the custom [SeqA_ind] (which gives the
    correct [TR1]-wrapped IH for the polymorphic spine recursion).

    The [MoreA] case is admitted.  The cascade through [ftailA']'s
    four Pair-head sub-cases produces three recursive obligations
    of the form [ftailA' (MoreA f0 m0 r2) ≤ ftailA' (MoreA f2 m2 r3)].
    Each should close by applying the [TR1]-extracted IH ([IH_inner])
    to the structural hypothesis between the two [MoreA]s.  However,
    [solve_mon] (from [ApproxM.v]) aggressively clears its inversion
    residue, erasing the equation chain that links the IH's universal
    [x6] (extracted from [Thunk x6 = mD2]) to the goal's concrete
    [MoreA f2 m2 r3] (obtained from destructuring [s0] earlier in the
    same chain).  Without that equation, [apply IH_inner] fails
    unification.

    Mathematically the result holds: it is a direct structural fact
    about the cascade, and is confirmed by Lorenzen 2025's
    credit-monad analysis of the persistent setting.  Mechanizing it
    appears to require either a custom monotonicity tactic that
    preserves [solve_mon]'s inversion residue, or hand-written
    application of [forcing_mon] / [bind_mon] at every cascade level
    so that the recursive site is reached with full context intact.
    Both are several days of tactic engineering.  Deferred. *)
